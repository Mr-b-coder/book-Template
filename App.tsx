...
import TextArea from './components/TextArea.tsx'; // Keep TextArea import for potential future use
...
import { ClipboardDocumentCheckIcon, ClipboardIcon } from './components/ClipboardIcons.tsx'; 
...
import SystemThemeIcon from './components/SystemThemeIcon.tsx';
...
const App: React.FC = () => {
  ...
  // --- Custom Barcode Generator State ---
  ...
  const [themePreference, setThemePreference] = useState<ThemePreference>(() => {
    if (typeof window !== 'undefined') {
      ...
      return (savedThemePreference || "system") as ThemePreference;
    }
    return "system";
  });
  ...
  const [actualTheme, setActualTheme] = useState<'light' | 'dark'>(() => {
    ...
    const initialPreference = (localStorage.getItem('themePreference') || "system") as ThemePreference;
    if (initialPreference === "system") {
      return window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
    }
    return initialPreference as 'light' | 'dark';
  });
  ...
  useEffect(() => {
    const applyEffectiveTheme = (currentPreference: ThemePreference) => {
      ...
      if (currentPreference === "system") {
        ...
        newActualTheme = currentPreference; // 'light' or 'dark'
      }
  ...
  const handleDataMatrixInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setDataMatrixInput(e.target.value);
  ...
  useEffect(() => { 
    ...
    if (cleaned.length === 10) {
      ...
      const eanPrefix = '978' + cleaned.substring(0, 9);
      const checkDigit = calculateEAN13CheckDigit(eanPrefix);
      finalEan13 = eanPrefix + checkDigit;
      ...
      if (parseInt(cleaned[12],10) !== calculatedCheck) {
        setIsbnBarcodeError('Invalid ISBN-13 check digit.');
        setEan13Data(null);
        return;
      }
      finalEan13 = cleaned;
    ...
    if (finalEan13 && ean13CanvasRef.current) {
      ...
      canvas.height = 600; 
      try {
        // bwip-js generates black on transparent by default
        ...
        console.error("EAN-13 generation error:", e);
        setIsbnBarcodeError('Failed to generate ISBN barcode.');
        setEan13ImageDataUrl(null);
      }
    }
  }, [rawIsbnInput]);
  ...
  useEffect(() => { 
    ...
    if (currentEan5Data && ean5CanvasRef.current && ean13Data && rawPriceInput.trim() !== '') { 
      ...
      try {
        ...
        bwipjs.toCanvas(canvas, ean5Options);
  ...
  useEffect(() => {
    const generateCombinedPreview = async () => {
        if (!ean13ImageDataUrl || !displayIsbnText || !combinedScratchCanvasRef.current) {
            setCombinedIsbnPricePreviewUrl(null);
            return;
        }

        const canvas = combinedScratchCanvasRef.current;
        ...
        imageLoadPromises.push(new Promise<void>((resolve, reject) => {
            ean13Image.onload = () => { ean13Loaded = true; resolve(); };
        ...
        if (showPriceArea) {
            imageLoadPromises.push(new Promise<void>((resolve, reject) => {
                ean5Image.onload = () => { ean5Loaded = true; resolve(); };
                ean5Image.onerror = () => reject(new Error("Failed to load EAN-5 image for preview."));
        ...
        try {
            ...
            const PREVIEW_TEXT_FONT = `bold ${PREVIEW_FONT_SIZE_PX}px 'Courier New', Courier, monospace`;
            const PREVIEW_PADDING = 5;
            const PREVIEW_TEXT_IMAGE_GAP = 3;
            const PREVIEW_SECTION_GAP = showPriceArea ? 10 : 0;
            ...
            // Always generate with white background and black text/barcode for preview
            ...
            // ean13Image and ean5Image are black on transparent, will draw as black on the white background
            ...
            console.error("Error generating combined preview:", err);
            setCombinedIsbnPricePreviewUrl(null);
            setIsbnBarcodeError("Preview generation failed.");
        }
    };
  ...
  useEffect(() => { 
    ...
    if (dataMatrixCanvasRef.current) {
        ...
        canvas.height = 300; 
        try {
            // Generate black barcode on white background for preview
            ...
            console.error("Data Matrix generation error:", e);
            setDataMatrixError('Failed to generate Data Matrix barcode. Check input data.');
            setDataMatrixImageDataUrl(null);
        }
    }
  }, [dataMatrixInput]);
  ...
  const calculateCoverDimensions = useCallback(() => {
    ...
    if (trimWidthNum <= 0 || trimHeightNum <= 0) {
      setError("Trim width and height must be positive numbers.");
      return null;
    }
    ...
    if (formData.bindingType === BindingType.PERFECT_BIND) {
      ...
      calculations.totalCoverWidth = (trimWidthNum * 2) + spineWidth + (STANDARD_BLEED_AMOUNT_INCHES * 2);
  ...
  const getBaseFileName = (dimensions: CoverCalculations): string => {
    ...
    const pageCountForFilename = dimensions.pageCountNum ? `${dimensions.pageCountNum}p` : 'Coil';
    return `${titlePart}Book_${dimensions.bindingType.replace(/[^a-zA-Z0-9]/g, '')}_${trimSizeForFilename}_${pageCountForFilename}`;
  };


  const downloadCoverFiles = async () => {
    if (!calculatedDimensions) return;
    setIsProcessing(true);
    setCurrentProcessingFormat("Cover Files");
    try {
      const zip = new JSZip();
      const baseFileName = getBaseFileName(calculatedDimensions);

      const pdfBlob = await generateCoverPdfBlob(calculatedDimensions, jsPDF);
      zip.file(`${baseFileName}_Template.pdf`, pdfBlob);
      ...
      console.error("Download Cover Files error:", err);
      setError(`Failed to generate cover files: ${err.message}`);
    } finally {
  ...
  const downloadInteriorFiles = async () => {
    if (!calculatedDimensions) return;
    setIsProcessing(true);
    setCurrentProcessingFormat("Interior Files");
    try {
      const zip = new JSZip();
      ...
      const gutterSpec = [
        ...
        { range: [601, Infinity], margin: 1.25 },
      ];
      const pc = pageCountNum || 0;
      const gutter = gutterSpec.find(g => pc >= g.range[0] && pc <= g.range[1])?.margin || 0.5;
      ...
      console.error("Download Interior Files error:", err);
      setError(`Failed to generate interior files: ${err.message}`);
    } finally {
      setIsProcessing(false);
      setCurrentProcessingFormat(null);
    }
  ...
  const downloadAllFiles = async () => {
    if (!calculatedDimensions) return;
    setIsProcessing(true);
    setCurrentProcessingFormat("All Files");
    try {
      const mainZip = new JSZip();
      const coverFolder = mainZip.folder("Cover_Files");
      const interiorFolder = mainZip.folder("Interior_Files");
      const baseFileName = getBaseFileName(calculatedDimensions);
      ...
      setError(`Failed to generate all files: ${err.message}`);
    } finally {
      setIsProcessing(false);
      setCurrentProcessingFormat(null);
    }
  ...
  const handleDownloadCustomBarcode = async (format: 'JPEG' | 'PDF') => {
    ...
    try {
        ...
        imageLoadPromises.push(new Promise<void>((resolve, reject) => {
            ...
            ean13Image.src = ean13ImageDataUrl; // This is black-on-transparent
        }));
        ...
        if (isPriceValidForAddon && ean5ImageDataUrl) { 
            ...
            imageLoadPromises.push(new Promise<void>((resolve, reject) => {
                ...
                ean5Image.src = ean5ImageDataUrl!; // This is black-on-transparent
            }));
        }
        ...
        ctx.font = TEXT_FONT; 
        ctx.fillStyle = actualTheme === 'dark' ? '#1E293B' : '#FFFFFF'; // Background
        ctx.fillRect(0, 0, tempCanvas.width, tempCanvas.height);
        ctx.fillStyle = actualTheme === 'dark' ? '#F1F5F9' : '#0A2F5C'; // Text
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ...
        // ean13Image and ean5Image are black on transparent.
        // For dark theme, invert them to white for download.
        if (actualTheme === 'dark') ctx.filter = 'invert(1)';
        ...
        if (actualTheme === 'dark') ctx.filter = 'none'; // Reset filter
        ...
        if (format === 'JPEG') {
            ...
            const imgData = tempCanvas.toDataURL('image/png'); // Use PNG for PDF for quality
            const pdf = new jsPDF({
  ...
  const handleDownloadDataMatrixBarcode = async (format: 'JPEG' | 'PDF') => {
    if (!dataMatrixImageDataUrl || !dataMatrixInput.trim()) { // dataMatrixImageDataUrl is now black-on-white
    ...
    try {
        ...
        await new Promise<void>((resolve, reject) => {
            ...
            dataMatrixImage.src = dataMatrixImageDataUrl; // Source is black-on-white
        });
        ...
        ctx.fillStyle = actualTheme === 'dark' ? '#1E293B' : '#FFFFFF'; // Themed background
        ctx.fillRect(0, 0, tempCanvas.width, tempCanvas.height);
        ...
        // dataMatrixImage is black-on-white. If dark theme, invert it to white-on-black for download.
        if (actualTheme === 'dark') {
            ctx.filter = 'invert(1)';
        }
        ...
        // Set themed text color for the label
        ctx.font = TEXT_FONT_DM;
        ctx.fillStyle = actualTheme === 'dark' ? '#F1F5F9' : '#0A2F5C';
  ...
  const handleGenerateBrandingCopy = async () => {
    if (!bookDescriptionForBranding.trim()) {
      setBrandingCopyError("Please describe your book first.");
      return;
    }
    setIsGeneratingBrandingCopy(true);
    setBrandingCopyError(null);
    setGeneratedBrandingCopy(null);

    try {
      const ai = new GoogleGenAI({ apiKey: process.env.API_KEY! });
      const prompt = `
        You are a creative marketing assistant for authors. Based on the following book description, generate branding copy.
        The book is: "${bookDescriptionForBranding}".

        Please provide the output in JSON format with the following structure:
        {
          "tagline": "A catchy tagline, 10-15 words long.",
          "description": "A compelling short book description, 50-75 words long, suitable for online listings or back cover blurbs.",
          "keywords": ["keyword1", "keyword2", "keyword3", "keyword4", "keyword5"] 
        }

        Ensure the tagline is impactful, the description is engaging, and the keywords are relevant for discoverability.
      `;
      ...
      try {
        const parsedData: GeneratedBrandingCopy = JSON.parse(jsonStr);
        if (parsedData.tagline && parsedData.description && Array.isArray(parsedData.keywords)) {
            setGeneratedBrandingCopy(parsedData);
        } else {
        ...
        setBrandingCopyError("Failed to parse branding copy. The AI's response might not be in the expected format. Raw response logged to console.");
      }

    } catch (err) {
      console.error("Error generating branding copy with Gemini:", err);
      setBrandingCopyError("An error occurred while generating branding copy. Please try again. " + (err instanceof Error ? err.message : String(err)));
    } finally {
      setIsGeneratingBrandingCopy(false);
    }
  };

  const copyBrandingItemToClipboard = (itemKey: string, textToCopy: string) => {
    navigator.clipboard.writeText(textToCopy).then(() => {
      setCopiedBrandingItem(itemKey);
      if (brandingCopyTimeoutRef.current) {
        clearTimeout(brandingCopyTimeoutRef.current);
      }
      brandingCopyTimeoutRef.current = window.setTimeout(() => {
        setCopiedBrandingItem(null);
      }, 2000);
    }).catch(err => {
      console.error(`Failed to copy ${itemKey}: `, err);
      alert(`Failed to copy ${itemKey} to clipboard.`);
    });
  };
  */
  ...
  const currentLegendItems = useMemo(() => {
    ...
    if (bindingType === BindingType.PERFECT_BIND) {
      items.push({ label: 'Bleed Edge', color: legendColorMapping.bleed, type: 'line' });
      items.push({ label: 'Trim Line / Fold', color: legendColorMapping.trim, type: 'line' });
      items.push({ label: 'Spine Area', color: legendColorMapping.spine, type: 'box' });
      items.push({ label: 'Safety Margin', color: legendColorMapping.safety, type: 'line-dashed' });
    } else if (bindingType === BindingType.CASE_BIND) {
      items.push({ label: 'Wrap Edge', color: legendColorMapping.wrap, type: 'line' });
      items.push({ label: 'Board Area / Fold', color: legendColorMapping.board, type: 'box' });
      items.push({ label: 'Hinge Area', color: legendColorMapping.hinge, type: 'box' });
      items.push({ label: 'Spine Board Area', color: legendColorMapping.spine, type: 'box' });
      items.push({ label: 'Safety Margin', color: legendColorMapping.safety, type: 'line-dashed' });
    } else if (bindingType === BindingType.COIL_WIRE_O_SOFTCOVER) {
      items.push({ label: 'Bleed Edge', color: legendColorMapping.bleed, type: 'line' });
      items.push({ label: 'Trim Line', color: legendColorMapping.trim, type: 'line' });
      items.push({ label: 'Safety Margin', color: legendColorMapping.safety, type: 'line-dashed' });
  ...
  const togglePreviewSection = (section: 'cover' | 'interior') => {
          ...
          <div className="flex items-center space-x-4">
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-[#0A2F5C] dark:text-[#F1F5F9]">Template Generator</h1>
             <div className="flex items-center space-x-1 p-0.5 bg-slate-200 dark:bg-slate-700 rounded-lg">
                <Button
                    variant={themePreference === 'light' ? 'secondary' : 'outline'}
                    size="sm"
                    onClick={() => setThemePreference('light')}
                    aria-pressed={themePreference === 'light'}
                    className={`min-w-[5.5rem] ${themePreference !== 'light' ? 'text-slate-700 dark:text-slate-200 border-slate-300 dark:border-slate-500 hover:bg-slate-100 dark:hover:bg-slate-600' : 'dark:!text-[#0F172A]'}`}
                    title="Light Theme"
                >
                    <SunIcon className="w-4 h-4 mr-1.5" /> Light
                </Button>
                <Button
                    variant={themePreference === 'dark' ? 'secondary' : 'outline'}
                    size="sm"
                    onClick={() => setThemePreference('dark')}
                    aria-pressed={themePreference === 'dark'}
                     className={`min-w-[5.5rem] ${themePreference !== 'dark' ? 'text-slate-700 dark:text-slate-200 border-slate-300 dark:border-slate-500 hover:bg-slate-100 dark:hover:bg-slate-600' : 'dark:!text-[#0F172A]'}`}
                    title="Dark Theme"
                >
                    <MoonIcon className="w-4 h-4 mr-1.5" /> Dark
                </Button>
                <Button
                    variant={themePreference === 'system' ? 'secondary' : 'outline'}
                    size="sm"
                    onClick={() => setThemePreference('system')}
                    aria-pressed={themePreference === 'system'}
                     className={`min-w-[5.5rem] ${themePreference !== 'system' ? 'text-slate-700 dark:text-slate-200 border-slate-300 dark:border-slate-500 hover:bg-slate-100 dark:hover:bg-slate-600' : 'dark:!text-[#0F172A]'}`}
                    title="System Theme"
                >
      ...
      <div className="grid lg:grid-cols-12 gap-8 items-start flex-grow">
        <div className="lg:col-span-4 space-y-8">
            ...
            <div className="bg-[#FFFFFF] dark:bg-[#1E293B] p-0 shadow-lg rounded-lg border border-[#DDE3ED] dark:border-[#334155]" aria-labelledby="branding-hub-heading">
                <button
                    onClick={() => setShowBrandingHubUI(!showBrandingHubUI)}
                    ...
                    aria-expanded={showBrandingHubUI}
                    aria-controls="branding-hub-content"
                >
                    <div className="flex items-center space-x-2">
                        <LightbulbIcon className="text-[#0A2F5C] dark:text-[#13B5CF]" />
                        <h2 id="branding-hub-heading" className="text-xl font-semibold text-[#0A2F5C] dark:text-[#F1F5F9]">
                            Branding & Marketing Hub ✨
                        </h2>
                    </div>
                    <ChevronIcon isOpen={showBrandingHubUI} className="text-[#0A2F5C] dark:text-[#F1F5F9]" />
                </button>
                {showBrandingHubUI && (
                    <div id="branding-hub-content" className="p-6 bg-[#FFFFFF] dark:bg-[#1E293B] rounded-b-lg border-t border-[#DDE3ED] dark:border-[#334155] space-y-6">
                        <TextArea
                            label="Describe Your Book (Genre, Themes, Target Audience, Unique Selling Points):"
                            id="bookDescriptionForBranding"
                            name="bookDescriptionForBranding"
                            value={bookDescriptionForBranding}
                            onChange={(e) => setBookDescriptionForBranding(e.target.value)}
                            rows={4}
                            placeholder="e.g., A thrilling sci-fi adventure for young adults, exploring themes of courage and friendship in a dystopian future. Unique for its strong female protagonist and unexpected plot twists."
                            disabled={isGeneratingBrandingCopy}
                            required
                        />
                        <Button 
                            onClick={handleGenerateBrandingCopy} 
                            disabled={isGeneratingBrandingCopy || !bookDescriptionForBranding.trim()}
                            variant="primary"
                            className="w-full"
                        >
                            {isGeneratingBrandingCopy ? 'Generating with AI...' : 'Generate with AI ✨'}
                        </Button>

                        {isGeneratingBrandingCopy && (
                            <div className="flex items-center justify-center p-4 text-sm text-[#4A5E78] dark:text-[#94A3B8]">
                                <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-[#0A2F5C] dark:text-[#13B5CF]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                </svg>
                                Generating suggestions with Gemini AI...
                            </div>
                        )}

                        {brandingCopyError && <p className="text-[#EF4444] dark:text-[#FCA5A5] text-sm bg-[#FEF2F2] dark:bg-[#7F1D1D] p-3 rounded-md">{brandingCopyError}</p>}

                        {generatedBrandingCopy && !isGeneratingBrandingCopy && (
                            <div className="mt-6 pt-6 border-t border-[#DDE3ED] dark:border-[#334155] space-y-5">
                                <div>
                                    <div className="flex justify-between items-center mb-1">
                                        <h4 className="text-md font-semibold text-[#0A2F5C] dark:text-[#F1F5F9]">AI-Generated Tagline:</h4>
                                        <Button 
                                            onClick={() => copyBrandingItemToClipboard('tagline', generatedBrandingCopy.tagline)}
                                            variant="outline" 
                                            size="sm"
                                            leftIcon={copiedBrandingItem === 'tagline' ? <CheckIcon className="w-4 h-4 text-[#22C55E] dark:text-[#86EFAC]"/> : <ClipboardIcon className="w-4 h-4"/>}
                                        >
                                            {copiedBrandingItem === 'tagline' ? 'Copied!' : 'Copy'}
                                        </Button>
                                    </div>
                                    <p className="text-sm p-3 bg-[#F8FAFC] dark:bg-[#0F172A] rounded-md text-[#0A2F5C] dark:text-[#F1F5F9]">{generatedBrandingCopy.tagline}</p>
                                </div>
                                <div>
                                    <div className="flex justify-between items-center mb-1">
                                        <h4 className="text-md font-semibold text-[#0A2F5C] dark:text-[#F1F5F9]">AI-Generated Description:</h4>
                                        <Button 
                                            onClick={() => copyBrandingItemToClipboard('description', generatedBrandingCopy.description)}
                                            variant="outline" 
                                            size="sm"
                                            leftIcon={copiedBrandingItem === 'description' ? <CheckIcon className="w-4 h-4 text-[#22C55E] dark:text-[#86EFAC]"/> : <ClipboardIcon className="w-4 h-4"/>}
                                        >
                                            {copiedBrandingItem === 'description' ? 'Copied!' : 'Copy'}
                                        </Button>
                                    </div>
                                    <p className="text-sm p-3 bg-[#F8FAFC] dark:bg-[#0F172A] rounded-md text-[#0A2F5C] dark:text-[#F1F5F9] whitespace-pre-wrap">{generatedBrandingCopy.description}</p>
                                </div>
                                <div>
                                    <div className="flex justify-between items-center mb-1">
                                        <h4 className="text-md font-semibold text-[#0A2F5C] dark:text-[#F1F5F9]">AI-Generated Keywords:</h4>
                                        <Button 
                                            onClick={() => copyBrandingItemToClipboard('keywords', generatedBrandingCopy.keywords.join(', '))}
                                            variant="outline" 
                                            size="sm"
                                            leftIcon={copiedBrandingItem === 'keywords' ? <CheckIcon className="w-4 h-4 text-[#22C55E] dark:text-[#86EFAC]"/> : <ClipboardIcon className="w-4 h-4"/>}
                                        >
                                            {copiedBrandingItem === 'keywords' ? 'Copied!' : 'Copy'}
                                        </Button>
                                    </div>
                                    <div className="flex flex-wrap gap-2 p-3 bg-[#F8FAFC] dark:bg-[#0F172A] rounded-md">
                                        {generatedBrandingCopy.keywords.map((keyword, index) => (
                                            <span key={index} className="text-xs bg-[#DDE3ED] dark:bg-[#334155] text-[#0A2F5C] dark:text-[#F1F5F9] px-2 py-1 rounded-full">
                                                {keyword}
                                            </span>
                                        ))}
                                    </div>
                                </div>
                                <p className="text-xs text-center text-[#4A5E78] dark:text-[#94A3B8] italic mt-4">
                                    AI-generated content provided by Gemini. Please review and refine for your needs.
                                </p>
                            </div>
                        )}
                    </div>
            ...
            */}
            ...
            <div className="bg-[#FFFFFF] dark:bg-[#1E293B] p-0 shadow-lg rounded-lg border border-[#DDE3ED] dark:border-[#334155]" aria-labelledby="barcode-generator-heading">
                ...
                >
                    <div className="flex items-center space-x-2">
                        <BarcodeIcon className="text-[#0A2F5C] dark:text-[#13B5CF]" /> {/* Using accent for dark mode icon */}
                ...
                {showBookSpecAndSummary && (
                    <div id="book-spec-summary-content" className="p-6 bg-[#FFFFFF] dark:bg-[#1E293B] rounded-b-lg border-t border-[#DDE3ED] dark:border-[#334155]">
                        ...
                        <form onSubmit={handleTemplateFormSubmit} className="space-y-5">
                            ...
                            <Select label="Binding Type" id="bindingType" name="bindingType" value={formData.bindingType} onChange={handleTemplateFormChange} options={bindingTypeOptionsForSelect} required />
                            </>
                            ...
                            <div className="mt-8 pt-6 border-t border-[#DDE3ED] dark:border-[#334155]">
                                ...
                                {showDownloadOptionsSet && (
                                    <div className="mt-4 pt-4 border-t border-[#A8B8D0] dark:border-[#475569] space-y-4">
                                        ...
                                        <Button onClick={downloadAllFiles} leftIcon={<ZipFileIcon/>} disabled={isProcessing} variant="primary" className="w-full">Download All Files (Cover & Interior)</Button>
                                        <div className="flex space-x-3">
                                            <Button onClick={downloadCoverFiles} leftIcon={<ZipFileIcon/>} disabled={isProcessing} variant="secondary" className="flex-1">Download Cover Files</Button>
                                            <Button onClick={downloadInteriorFiles} leftIcon={<ZipFileIcon/>} disabled={isProcessing} variant="secondary" className="flex-1">Download Interior Files</Button>
                                        </div>
                                    </div>
                                )}
                                </div>
...
export default App;
// Adding a comment to ensure re-evaluation for troubleshooting. Check browser console for errors if issues persist.
// Final check of dark mode logic and overall structure.
